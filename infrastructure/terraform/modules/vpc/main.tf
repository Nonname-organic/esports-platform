...
# ── Private Subnets (optional — MVP/prod only) ────────────────────────────────
...
# ── NAT Gateway (optional — ~$32/month, MVP/prod only) ───────────────────────
...
resource "aws_security_group" "ec2" {
  ...
  description = "HTTP/HTTPS public access + SSH from admin IP"
  vpc_id      = aws_vpc.this.id
  ...
  # SSH access restricted by the admin_cidr variable at environment level.
  # Default is empty — add your IP in terraform.tfvars.
  dynamic "ingress" {
...
# RDS — only reachable from EC2/ECS security group
resource "aws_security_group" "rds" {
  name        = "${local.name_prefix}-sg-rds"
  description = "PostgreSQL access from application layer only"
  vpc_id      = aws_vpc.this.id
...
# ElastiCache (MVP) — only from app layer
resource "aws_security_group" "redis" {
  name        = "${local.name_prefix}-sg-redis"
  description = "Redis access from application layer only"
  vpc_id      = aws_vpc.this.id
