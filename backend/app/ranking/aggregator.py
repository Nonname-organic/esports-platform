...
from app.ranking.tiers import PLACEMENT_RP, next_tier_for, tier_for, tier_progress
...
class RankingAggregator:
    ...
    async def global_team_leaderboard(
        ...
        cache_key = f"cache:ranking:global:{game or 'all'}:{season}"
    ...
    async def _compute(self, *, game: Optional[str], season: str) -> list[dict]:
        ...
        for tid in tournament_ids:
            ...
            for team_id, label in placements.items():
                a = agg.setdefault(team_id, {"rp": 0, "tournaments": 0, "championships": 0, "wins": 0, "losses": 0})
